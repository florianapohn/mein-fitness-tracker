...
if check_password():
    ...
    with conn.session as session:
        # Falls die Tabelle noch nicht existiert (oder zurückgesetzt wurde)
    ...
    with tab1:
        if not df_filled.empty:
            # Erhöhe den Zeitraum auf 365 Tage, damit auch ältere Backup-Daten direkt im Trend-Tab sichtbar sind
            ten_days_ago = pd.Timestamp.now() - pd.Timedelta(days=365)
            ...
            if uploaded_file is not None:
                try:
                    ...
                    with conn.session as session:
                        ...
                        session.commit()
                    st.cache_data.clear() # Leert den Cache, damit Streamlit die hochgeladenen Daten sofort neu abfragt!

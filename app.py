...
# Einstellungen laden (für die Anzeige in der Sidebar)
...
with st.sidebar.form("entry_form", clear_on_submit=True):
    # Hier ist das Datum auf 'date.today()' voreingestellt
    ...
    k_in = st.number_input("Kalorien (Gegessen)", step=50, min_value=0)
    k_out = st.number_input("Kalorien (Verbrannt)", step=50, min_value=0)
    
    st.subheader("📏 Körpermaße (cm)")
    hals = st.number_input("Hals", format="%.1f", min_value=0.0)
    brust = st.number_input("Brust", format="%.1f", min_value=0.0)
    bauch = st.number_input("Bauch", format="%.1f", min_value=0.0)
    bein = st.number_input("Oberschenkel", format="%.1f", min_value=0.0)
...
# 4. SEITENLEISTE: Benachrichtigungen
st.sidebar.markdown("---")
